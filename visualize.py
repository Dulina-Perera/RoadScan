...
# %%
def extract_license_plates(cap: VideoCapture, results: DataFrame) -> Dict:
	license_plates: Dict = {}
	...
	for vehicle_id in results['Vehicle_ID'].unique():
		...
		x1, y1, x2, y2 = ast.literal_eval(results[(results['Vehicle_ID'] == vehicle_id) & (results['License_Number_Score'] == max_score)]['License_Plate_Bbox'].iloc[0])

		license_plate_crop: ndarray = frame[int(y1):int(y2), int(x1):int(x2), :]
		license_plate_crop: ndarray = cv2.resize(license_plate_crop, (int((x2 - x1) * 100 / (y2 - y1)), 100))
...
def draw(cap: VideoCapture, results: DataFrame, license_plates: Dict) -> None:
	frame_no: int = -1
	...
	while ret:
		...
		if ret:
			...
			for row_idx in range(len(df_temp)):
				...
				try:
					frame[int(vehicle_y1) - H - 50:int(vehicle_y1) - 50, int((vehicle_x2 + vehicle_x1 - W) / 2):int((vehicle_x2 + vehicle_x1 + W) / 2), :] = license_plate_crop
					frame[int(vehicle_y1) - H - 150:int(vehicle_y1) - H - 50, int((vehicle_x2 + vehicle_x1 - W) / 2):int((vehicle_x2 + vehicle_x1 + W) / 2), :] = (255, 255, 255)

					(text_width, text_height), _ = cv2.getTextSize(license_plates[df_temp.iloc[row_idx]['Vehicle_ID']]['license_number'], cv2.FONT_HERSHEY_SIMPLEX, 1, 3)
					cv2.putText(
						frame,
						license_plates[df_temp.iloc[row_idx]['Vehicle_ID']]['license_number'],
						(int((vehicle_x2 + vehicle_x1 - text_width) / 2), int(vehicle_y1 - H - 100 + (text_height / 2))),
...
if __name__ == '__main__':
  ...
  license_plates: Dict = extract_license_plates(cap, results)

	# Draw bounding boxes for vehicles and license plates.
  draw(cap, results, license_plates)
